...
`push` и `pull` пропускают неизмененные файлы: сначала сравнивают размер, затем md5/sha256. Если включен `skip-hash = yes`, после совпадения размера хеши не проверяются.
`push` также пропускает пустые локальные файлы и пишет `Skipped empty`.

Для одной команды можно включить быстрый режим без хешей:
